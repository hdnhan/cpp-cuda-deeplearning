...
int main() {
    ...
    T *hat = (T *)malloc(arows * N * sizeof(T));  // tranpose
    ...
    // Tranpose
    multiplication::transpose(ha, hat, arows, N);

    // Allocate device memory
    T *dat, *db, *dc;
    cudaMalloc(&dat, arows * N * sizeof(T));
    cudaMalloc(&db, N * bcols * sizeof(T));
    cudaMalloc(&dc, arows * bcols * sizeof(T));

    // Copy data to the device
    cudaMemcpy(dat, hat, arows * N * sizeof(T), cudaMemcpyHostToDevice);
    ...
    // Launch kernel
    multiplication::mul_cols<<<blocks, threads>>>(dat, db, dc, arows, N, bcols);
